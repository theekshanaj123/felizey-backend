...
exports.addNewRole = async (req, res) => {
  try {
    ...
    const resquredFields = ["email", "role", "event_id"];
...
exports.acceptRoll = async (req, res) => {
  try {
    const { email, role, event_id } = req.body;
    ...
    if (isExist) {
      ...
      return res.status(400).json({
        message: "This user haven't a role in this event.",
      });
    }
  } catch (e) {
    console.error(e);
...
exports.declineRoll = async (req, res) => {
  try {
    const { email, role, event_id } = req.body;
...
exports.getUserRolls = async (req, res) => {
  try {
    const userRolls = await prisma.user_Role.findMany({
    ...
    if (userRolls) {
      ...
      for (const userR of userRolls) {
        const event = await prisma.event.findFirst({
          where: { id: userR.event_id },
        });
        const json = {
          id: userR.id,
          title: event.title,
          ...
          event_id: userR.event_id,
          created_at: userR.created_at,
          imageUrl: event.image_url,
        };
        userEventRoll.push(json);
      }
...
exports.getUserRoleByEventId = async (req, res) => {
  try {
    ...
    const resquredFields = ["event_id", "email"];
