...
exports.scanTicketByEmailOrMobile = async (req, res) => {
  try {
    ...
    if (mobile) {
      ...
      if (user) {
        const ticket = await prisma.order_Item.findUnique({
  ...
  try {
    ...
    for (const ticket of ticketData) {
      ...
      if (ticket.qty_available === 0) {
        return res.status(400).json({
          message: ticket.type + " Tickets Not Available. Please Try Again.",
        });
      }
      ...
      if (ticket.qty_available < ticketD.quantity_available) {
        return res.status(400).json({
          message:
            ticket.type + " Tickets Quantity Not Available. Please Try Again.",
        });
      }

      if (
        ticket.qty_available === 1 ||
        ticket.qty_available === ticketD.quantity_available
      ) {
...
exports.bookTicket = async (req, res) => {
  try {
    const { tickets, event_id, total_amount, payment_status, currency } =
      req.body;
